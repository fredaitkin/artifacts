            ...
            <div class="tab-pane fade show active" id="batting" role="tabpanel" aria-labelledby="gatting-tab">
                ...
                <div class="ml-3 mt-3 p-3 w-75 border row">
                    <div class="col">
                        ...
                        <a href="/player/{{$most_home_runs->id}}?view=true">{{$most_home_runs->first_name}} {{$most_home_runs->last_name}}</a> {{$teams[$most_home_runs->team]}} <strong>{{$most_home_runs->home_runs}}</strong>
                    </div>
                    <div class="col">
                        <h6><strong>Most RBIs</strong></h6>
                        <a href="/player/{{$most_rbis->id}}?view=true">{{$most_rbis->first_name}} {{$most_rbis->last_name}}</a> {{$teams[$most_rbis->team]}} <strong>{{$most_rbis->rbis}}</strong>
                    </div>
                    <div class="col">
                        <h6><strong>Best Average</strong></h6>
                        <a href="/player/{{$best_average->id}}?view=true">{{$best_average->first_name}} {{$best_average->last_name}}</a> {{$teams[$best_average->team]}} <strong>{{$best_average->average}}</strong></br>
                ...
                <div class="ml-3 mt-3 p-3 w-50 border row">
                    <div class="col">
                        ...
                        <a href="/player/{{$best_hr_strike_rate->id}}?view=true">{{$best_hr_strike_rate->first_name}} {{$best_hr_strike_rate->last_name}}</a> {{$teams[$best_hr_strike_rate->team]}} <strong>{{$best_hr_strike_rate->strike_rate}}</strong></br>
                        <small class="text-muted">* At bats per home run</small>
                    </div>
                    <div class="col">
                        <h6><strong>Best RBI Strike Rate</strong></h6>
                        <a href="/player/{{$best_rbi_strike_rate->id}}?view=true">{{$best_rbi_strike_rate->first_name}} {{$best_rbi_strike_rate->last_name}}</a> {{$teams[$best_rbi_strike_rate->team]}} <strong>{{$best_rbi_strike_rate->strike_rate}}</strong></br>
                        ...
                        @foreach($most_home_runs_by_position as $k => $v)
                            @if(isset($v->id))
                                <div class="row">
                                    ...
                                    <div class="col">{{$teams[$v->team]}}</div>
            ...
            <div class="tab-pane fade" id="pitching" role="tabpanel" aria-labelledby="pitching-tab">
                ...
                <div class="ml-3 mt-3 p-3 w-75 border row">
                    <div class="col">
                        ...
                        <a href="/player/{{$most_wins->id}}?view=true">{{$most_wins->first_name}} {{$most_wins->last_name}}</a> {{$teams[$most_wins->team]}} <strong>{{$most_wins->wins}}</strong>
                    </div>

                    <div class="col">
                        <h6><strong>Best ERA</strong></h6>
                        <a href="/player/{{$best_era->id}}?view=true">{{$best_era->first_name}} {{$best_era->last_name}}</a> {{$teams[$best_era->team]}} <strong>{{$best_era->era}}</strong></br>
                    ...
                    <div class="col">
                        ...
                        <a href="/player/{{$best_win_strike_rate->id}}?view=true">{{$best_win_strike_rate->first_name}} {{$best_win_strike_rate->last_name}}</a> {{$teams[$best_win_strike_rate->team]}} <strong>{{$best_win_strike_rate->strike_rate}}</strong></br>
